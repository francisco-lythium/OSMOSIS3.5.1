...
byte intentosdereinicio = 0; //esta variable cuenta es para intentar arrancar la maquina despues de una falla.
...
void loop() {
	...
	else if (botononoff == 0)
	{
		...
		intentosdereinicio = 0;//si la maquina intento arrancar 3 veces, aqui se reinicia ese contador.
	}
...
void lecturasensores(void)
{
   ...
   // posicionvalvula    = map(ads3.readADC_SingleEnded(1),618,24391,0,2000); //ADS #3 valor antiguo 17536
	...
	caudalimetrodos = (Wire.read() / 10.0);
